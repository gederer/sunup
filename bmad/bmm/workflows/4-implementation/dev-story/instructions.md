...
<critical>MANDATORY: Git workflow procedures MUST be followed at all times - see {project-root}/docs/git-workflow.md</critical>

<workflow>

  <step n="0" goal="MANDATORY: Verify git safety and load workflow rules" tag="git-safety">
    <critical>This step is MANDATORY and CANNOT be skipped. It protects against data loss.</critical>

    <action>Read COMPLETE file: {project-root}/docs/git-workflow.md</action>
    <action>This file contains MANDATORY git procedures that MUST be followed</action>

    <action>Check for uncommitted changes by running: git status --porcelain</action>

    <check if="uncommitted changes exist">
      <output>⚠️  **MANDATORY CHECKPOINT REQUIRED**

You have uncommitted changes that MUST be committed before starting story implementation.
...
2. Commit all changes: `git add . && git commit -m "wip: checkpoint before Story X.Y"`
3. Verify clean state: `git status`
4. Then re-run this workflow

**Why This Matters:**
- Prevents data loss if story implementation fails
- Enables easy rollback to known good state
- Follows mandatory git workflow in docs/git-workflow.md

**This is NON-NEGOTIABLE. Story implementation cannot proceed with uncommitted changes.**
      </output>
      <ask>Would you like me to help you commit these changes now? (yes/no)</ask>

      <check if="user says yes">
        <action>Guide user through committing changes:
          1. Show git diff summary
          2. Suggest appropriate commit message
      ...
      <check if="user says no">
        <output>Cannot proceed without committing changes. Please commit manually and re-run this workflow.</output>
        <action>HALT</action>
      </check>
    </check>

    <check if="no uncommitted changes">
      <output>✅ Git state clean - safe to proceed with story implementation</output>
    </check>

    <action>Check current branch name: git branch --show-current</action>
    <action>Store current_branch for story branch creation</action>

    <output>📋 **Git Workflow Safety Check Complete**

Current branch: {{current_branch}}
Git state: Clean ✅
Ready to proceed with story implementation.

**Story Branch Strategy:**
- Story branch will be created: story/X-Y-story-name
- Frequent WIP commits will be made during implementation
- Final merge back to {{current_branch}} when story complete

Refer to docs/git-workflow.md for complete procedures.
    </output>
  </step>
